"""
PCO TechOps - Shift Capacity Planning Web Application
=====================================================
...
def compute_capacity_plan(forecaster, start_date, days=90, target_util=0.65):
    ...
    for d in range(days):
        ...
        for shift in ['S1', 'S2', 'S3']:
            ...
            # Required capacity at target utilization for MEAN demand
            # This ensures Expected_Util_% lands at ~65%
...
def generate_html_dashboard(data, plan_df, forecaster, team_id, team_name, partner_name):
    ...
    sim_sufficient = len(plan_df[plan_df['Expected_Util_%'] <= 65])
    ...
    weekly_template = plan_df.groupby(['DayOfWeekNum', 'Day', 'Shift']).agg({
        ...
        'Forecast_Hours': 'mean', 'Expected_Util_%': 'mean',
...
<head>
    ...
    <title>PCO TechOps - Shift Capacity Report | {partner_name}</title>
...
<div class="insight-box">
    ...
    65% utilization on average demand.
</div>
<div class="chart-card" style="margin-top:16px; overflow-x:auto;">
    <table>
        <thead><tr><th>Day</th><th>Shift</th><th>Current Avg</th><th>Recommended</th><th>Gap</th><th>Forecast Mean</th><th>Expected Util%</th></tr></thead>
        <tbody>{table_rows}</tbody>
    </table>
</div>
<div class="footer">
    PCO TechOps Shift Capacity Planning Tool | Model: Day-of-Week + Shift + Week-of-Month | Target: Mean demand at 65% utilization
</div>
<script>
...
def generate_excel_output(data, plan_df, forecaster):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        # Single sheet: 90-Day Plan with essential columns only
        plan_export = plan_df[['Date', 'Day', 'Shift',
                               'Recommended_Executors', 'Total_Capacity_Hrs',
                               'Forecast_Hours', 'Expected_Util_%', 'Buffer_Hours']].copy()
...
def main():
    ...
    with st.sidebar:
        ...
        **Shift Schedule:**
        ...
        st.markdown("---")
        st.markdown("**Target:** Mean demand at 65% utilization")
