...
def compute_capacity_plan(forecaster, start_date, days=90, target_util=0.65):
    ...
    for d in range(days):
        ...
        for shift in ['S1', 'S2', 'S3']:
            ...
            # This ensures Expected_Util_Mean lands at ~65%
...
def generate_html_dashboard(data, plan_df, forecaster, team_id, team_name, partner_name):
    ...
    sim_sufficient = len(plan_df[plan_df['Expected_Util_Mean'] <= 65])
    ...
    weekly_template = plan_df.groupby(['DayOfWeekNum', 'Day', 'Shift']).agg({
        ...
        'Forecast_Mean': 'mean', 'Expected_Util_Mean': 'mean',
    ...
    for _, row in weekly_template.iterrows():
        ...
        <tr>
            ...
            <td>{int(row['Expected_Util_Mean'])}%</td>
        </tr>"""
...
<div class="footer">
    ECS TechOps Shift Capacity Planning Tool | Model: Day-of-Week + Shift + Week-of-Month | Target: Mean demand at 65% utilization
</div>
<script>
...
def generate_excel_output(data, plan_df, forecaster):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        plan_export = plan_df[['Date', 'Day', 'Shift',
                               ...
                               'Expected_Util_Mean', 'Buffer_Hours']].copy()
        ...
        if st.session_state.get('processed', False):
            ...
            for dow in range(7):
                for shift in ['S1', 'S2', 'S3']:
                    ...
                    util = subset['Expected_Util_Mean'].mean()
